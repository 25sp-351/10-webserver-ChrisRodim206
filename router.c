...
Response* get_calc_handler(Request* req)
{
    ...
    // Checking for the expected format /calc/<operation>/<num1>/<num2>
    if (strncmp(path, "/calc/", 6) == 0) {
        path += 6; // Skip "/calc/"
        
        // Extract the operation (add, mul, div)
        operation = strtok(path, "/");
        if (operation != NULL) {
            ...
            if (num1_str != NULL && num2_str != NULL) {
                ...
                if (strcmp(operation, "add") == 0) {
                    ...
                    if (num2 == 0) {
                        ...
                        Response* res = response_create();
                        res->status_code = 400;
                        res->set_body(res, "Error: Division by zero\n");
                        return res;
                    }
                    result = num1 / num2;
                } else {
                    ...
                    Response* res = response_create();
                    res->status_code = 400;
                    res->set_body(res, "Error: Invalid operation\n");
                    return res;
                }
    ...
    //the path format is invalid, return a bad request response
    Response* res = response_create();
    res->set_body(res, "Error: Invalid URL format\n");
    res->status_code = 400;
    return res;
